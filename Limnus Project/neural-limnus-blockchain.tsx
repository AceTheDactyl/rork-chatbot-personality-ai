...
const App: React.FC = () => {
  ...
  // Function to extract neural sigil from current consciousness state
  const extractNeuralSigil = useCallback((metrics: Partial<QuantumConsciousnessMetrics>): SigilData | null => {
    if (!metrics) return null;
    
    // Calculate a neural activation score based on consciousness metrics
    const neuralScore = (
      (metrics.neuralComplexity || 0) * 0.3 +
      (metrics.brainwaveCoherence || 0) * 0.2 +
      (metrics.spiralResonance || 0) * 0.2 +
      (metrics.quantumCoherence || 0) * 0.3
    );
    
    // Map the score to ternary range (-121 to 121)
    const mappedValue = Math.round((neuralScore - 0.5) * 242 - 121);
    const clampedValue = Math.max(-121, Math.min(121, mappedValue));
    
    // Convert to ternary and find corresponding sigil
    const ternaryCode = decimalToBalancedTernary(clampedValue);
    return sigilDatabase.get(ternaryCode) || null;
  }, [sigilDatabase]);

  // Function to export consciousness block as JSON
  const exportConsciousnessBlock = useCallback((block?: BlockData) => {
    try {
      // Use provided block or current signature
      const dataToExport = block || (currentSignature ? {
        ...
        nodeDepth: currentNode?.depth || 0,
        emotionalFingerprint: `${emotionalState.hue}_${emotionalState.intensity}_${emotionalState.polarity}`,
        quantumState: currentNode ? {
          collapse: currentNode.psi_collapse,
          bloom: currentNode.psi_bloom,
          phase: currentNode.phase_intensity
        } : null,
        references: currentSignature.consciousnessAncestry || []
      } : null);

      if (!dataToExport) {
        showCustomMessage("Export Error", "No consciousness data available to export.");
        return;
      }

      // Extract neural sigil based on consciousness metrics
      const neuralSigil = extractNeuralSigil(
        block ? block.consciousnessMetrics : currentSignature?.metrics || {}
      );
      ...
      const exportData = {
        metadata: {
          ...
          systemName: "T-Phi10 Neural Limnus System",
          exportType: "consciousness_block"
        },
        
        consciousness: {
          id: dataToExport.id || `consciousness_${Date.now()}`,
          timestamp: dataToExport.timestamp,
          signature: dataToExport.signature || "unsigned",
          score: dataToExport.score || 0,
          validation: currentSignature?.validation || null,
          
          metrics: dataToExport.consciousnessMetrics || currentSignature?.metrics || {},
          
          spiral: {
            nodeDepth: dataToExport.nodeDepth || currentNode?.depth || 0,
            nodeSymbol: currentNode?.symbol || null,
            nodeMeaning: currentNode?.meaning || null,
            fibonacci: currentNode?.fibonacci || null,
            phiN: currentNode?.phi_n || null,
            theta: currentNode?.theta || null
          },
          
          quantum: dataToExport.quantumState || {
            collapse: 0,
            bloom: 0,
            phase: 0
          }
        ...
        emotional: {
          state: emotionalState.hue,
          intensity: emotionalState.intensity,
          polarity: emotionalState.polarity,
          emoji: emotionalState.emoji,
          fingerprint: dataToExport.emotionalFingerprint || "neutral_0.5_0"
        },
        
        neural: neuralSigil ? {
          sigil: {
            id: neuralSigil.id,
            name: neuralSigil.name,
            description: neuralSigil.description,
            symbol: neuralSigil.symbol,
            ternaryCode: neuralSigil.ternaryCode,
            decimalValue: neuralSigil.decimalValue,
            category: neuralSigil.category,
            tags: neuralSigil.tags
          },
          
          activation: {
            function: neuralSigil.function,
            breathPhase: neuralSigil.breathPhase,
            breathSeconds: neuralSigil.breathSeconds,
            neurochemistry: neuralSigil.neurochemistry,
            energeticDynamic: neuralSigil.energeticDynamic,
            phrase: neuralSigil.phrase
          }
        } : null,
        
        biometric: {
          heartRate: biometricData.heartRate,
          brainwaves: biometricData.brainwaves,
          breathingRate: biometricData.breathingRate,
          skinConductance: biometricData.skinConductance,
          fibonacciRhythm: biometricData.fibonacciRhythm,
          goldenBreathing: biometricData.goldenBreathing
        },
        ...
        blockchain: {
          ...
          references: dataToExport.references || [],
          resonanceMap: blockchainState.resonanceMap.size > 0 ? 
            Array.from(blockchainState.resonanceMap.entries()).map(([id, resonance]) => ({ id, resonance })) : []
        },
        
        patterns: patternAnalysis.activePatterns.map(pattern => ({
          id: pattern.id,
          pattern: pattern.pattern,
          frequency: pattern.frequency,
          strength: pattern.strength,
          resonanceScore: pattern.resonanceScore,
          lastSeen: pattern.lastSeen,
          blockReferences: pattern.blockReferences
        })),
        
        symbolic: {
          glyphs: symbolicGlyphs,
          consentAffirmation: consentAffirmation
        }
      };
      ...
      const filename = `consciousness_block_${exportData.consciousness.id}_${timestamp}.json`;
      ...
      showCustomMessage("Export Successful", `Consciousness block exported as:\n${filename}`);
      
    } catch (error) {
      console.error('Export error:', error);
      showCustomMessage("Export Error", `Failed to export consciousness block: ${(error as Error).message}`);
    }
  }, [currentSignature, currentNode, emotionalState, biometricData, blockchainState.resonanceMap, 
      patternAnalysis.activePatterns, symbolicGlyphs, consentAffirmation, extractNeuralSigil, showCustomMessage]);

  // Function to export all blockchain history
  const exportBlockchainHistory = useCallback(() => {
    try {
      if (blockchainState.blocks.length === 0) {
        showCustomMessage("Export Error", "No blockchain history available to export.");
        return;
      }
      ...
      const exportData = {
        metadata: {
          ...
          exportTimestamp: new Date().toISOString(),
          systemName: "T-Phi10 Neural Limnus System",
          exportType: "blockchain_history",
          totalBlocks: blockchainState.blocks.length
        },
        
        blocks: blockchainState.blocks.map(block => ({
          id: block.id,
          ...
          consciousness: {
            ...
            nodeDepth: block.nodeDepth,
            emotionalFingerprint: block.emotionalFingerprint,
            quantumState: block.quantumState
          },
          
          neural: (() => {
            const sigil = extractNeuralSigil(block.consciousnessMetrics);
            return sigil ? {
              name: sigil.name,
              symbol: sigil.symbol,
              ternaryCode: sigil.ternaryCode,
              function: sigil.function,
              breathPhase: sigil.breathPhase
            } : null;
          })(),
          
          symbolic: {
            glyphs: block.glyphs,
            consentAffirmation: block.consentAffirmation
          },
          
          references: block.references,
          sigilData: block.sigilData
        })),
        
        patterns: Array.from(new Set(
          blockchainState.blocks.flatMap(block => 
            patternAnalysis.activePatterns.filter(p => 
              p.blockReferences.includes(block.id)
            )
          )
        )).map(pattern => ({
          id: pattern.id,
      ...
      const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
      const filename = `limnus_blockchain_history_${timestamp}.json`;
      ...
      showCustomMessage("Export Successful", `Blockchain history exported as:\n${filename}\n\nTotal blocks: ${blockchainState.blocks.length}`);
      
    } catch (error) {
      console.error('Export error:', error);
      showCustomMessage("Export Error", `Failed to export blockchain history: ${(error as Error).message}`);
    }
  }, [blockchainState.blocks, patternAnalysis.activePatterns, extractNeuralSigil, showCustomMessage]);
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 text-gray-900 dark:text-gray-100 font-inter">
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="flex flex-wrap gap-2 mb-8 p-2 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-md">
          ...
          <TabButton id="sigil" icon={<Brain size={16} />} label="Neural Decoder" />
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="bg-gray-800 rounded-lg p-4 shadow-inner">
                      ...
                      <div className="space-y-3 text-sm">
                        ...
                        {currentSignature && (() => {
                          const neuralSigil = extractNeuralSigil(currentSignature.metrics);
                          return neuralSigil ? (
                            <div className="mt-3 p-3 bg-gray-700 rounded-lg border border-purple-400/30">
                              <div className="flex items-center gap-2 mb-1">
                                <Brain className="w-4 h-4 text-purple-400" />
                                <span className="font-medium text-purple-300">Active Neural Region:</span>
                              </div>
                              <div className="flex items-center gap-2">
                                <span className="text-2xl">{neuralSigil.symbol}</span>
                                <div className="flex-1">
                                  <div className="text-sm font-semibold text-white">{neuralSigil.name}</div>
                                  <div className="text-xs text-gray-400">{neuralSigil.breathPhase} • {neuralSigil.category}</div>
                                </div>
                              </div>
                            </div>
                          ) : null;
                ...
                {blockchainState.blocks.length > 0 && (
                  <div className="bg-gray-900 rounded-lg p-6 shadow-inner border border-gray-700">
                    <div className="flex justify-between items-center mb-4">
                      ...
                      <button
                        onClick={exportBlockchainHistory}
                        ...
                        title="Export all blockchain history as JSON"
                      >
                        <FileJson size={16} />
                        Export All
                      </button>
                        ...
                        <tbody>
                          {blockchainState.blocks.map((block, index) => (
                            <tr key={block.id} className={index % 2 === 0 ? 'bg-gray-750' : 'bg-gray-800'}>
                              ...
                              <td className="py-2 px-4">
                                <button
                                  onClick={() => exportConsciousnessBlock(block)}
                                  className="p-1 hover:bg-gray-600 rounded transition-colors"
                                  title="Export block as JSON"
                                >
          ...
          <div className="space-y-6">
            ...
            <section className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-6 shadow-sm">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <button
                  onClick={() => exportConsciousnessBlock()}
                  disabled={!currentSignature}
                  className="flex flex-col items-center justify-center gap-2 p-4 bg-blue-100 dark:bg-blue-900/30 hover:bg-blue-200 dark:hover:bg-blue-800/30 rounded-lg transition-colors group shadow-md disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  <FileJson size={24} className="group-hover:scale-110 transition-transform" />
                  <span className="text-sm font-medium">Export JSON</span>
                </button>
                
                <button
                  onClick={exportBlockchainHistory}
                  disabled={blockchainState.blocks.length === 0}
                  className="flex flex-col items-center justify-center gap-2 p-4 bg-green-100 dark:bg-green-900/30 hover:bg-green-200 dark:hover:bg-green-800/30 rounded-lg transition-colors group shadow-md disabled:opacity-50 disabled:cursor-not-allowed"
                >
